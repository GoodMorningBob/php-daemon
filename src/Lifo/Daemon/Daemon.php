 ...
 * <br/>
 * <em>
 * The library has been rewritten to be PSR-4 compliant and requires PHP v5.4+. Certain features from the original
...
abstract class Daemon
{
     ...
     * (in seconds).
     *
     * This can help prevent memory leaks from crashing the daemon. Ultimately you should'nt rely on this feature
     * and should try and make sure your daemon does'nt leak memory, but in some cases, like when running your
     * application daemon in "debug" mode for long periods, you'll want it to restart occasionally.
     *
     * @var integer
     */
    private $autoRestartInterval = 0;
     ...
     * @var int
     */
    private $minRestartThreshold = 10;
     ...
     * @var bool
     */
    private $advancedLogChecks = false;
...
//      determine if it's growing memory usage overtime.
     ...
     */
    public static function blockSignals($signals, $callback)
    {
...
//            }
     ...
     * @return float
     */
    public function getLoopIterations()
    {
    ...
    {
        if ($interval !== null && (!is_numeric($interval) || $interval < 0.0)) {
            throw self::createOutOfBoundsException($interval, sprintf("loopInterval must be NULL or an integer >= 0; \"%s\" (%s) was given", $interval, gettype($interval)));
        }
     ...
     * @return int
     */
    public function getLoopSleepMin()
    {
     ...
     * @return $this
     */
    public function setLoopSleepMin($loopSleepMin)
    {
     ...
     * @return float
     */
    public function getIdleProbability()
    {
     ...
     * @return $this
     */
    public function setIdleProbability($probability)
    {
        if (!is_numeric($probability) || $probability < 0.0 || $probability > 1.0) {
        ...
        $this->idleProbability = (float)$probability;
        return $this;
    }
     ...
     * @return integer
     */
    public function getAutoRestartInterval()
    {
     ...
     * @return $this
     */
    public function setAutoRestartInterval($interval)
    {
        if ($interval !== null && (!is_numeric($interval) || $interval < 0)) {
        ...
        $this->autoRestartInterval = (int)$interval;
        return $this;
    }
     ...
     * @return $this
     */
    public function setMinRestartThreshold($interval)
    {
        ...
        $this->minRestartThreshold = (int)$interval;
        return $this;
    }
     ...
     * @return int
     */
    public function getMinRestartThreshold()
    {
     ...
     * @throws \LogicException if daemon has already been initialized
     * @return $this
     */
     ...
     */
    public function dispatch($eventName, DaemonEvent $event = null)
    {
        if ($event === null) {
//            $event = new DaemonEvent($this);
     ...
     * @param callable $callback  Callback. Must be a valid callable or \Closure
    ...
    /**
     * Return the total micro seconds the daemon has been running. Will be 0 until the {@link run} method is called.
     *
     ...
     * @return $this
     */
    public function setShutdown($flag = true)
    {
        if ($this->shutdown && !$flag) {
            $this->debug("Cannot disable shutdown flag once shutdown has started");
            return $this;
        }
        $this->shutdown = (bool)$flag;
        return $this;
    }
...
//        return sprintf("%s%04d: %d: ", date('Y-m-d H:i:s.', $time[0]), $time[1], $this->pid);
     ...
     *
     * @param array $stats Optional array of stats to dump. Defaults to all stats.
     */
...
//                'groups' => array_map(function ($id) { return posix_getgrgid($id)['name']; }, posix_getgroups()),
        ...
        /** @var StatsEvent $e */
        $e = $this->dispatch(DaemonEvent::ON_STATS, new StatsEvent($this, $stats));
        $stats = $e->getStats();
    ...
    {
        $this->debugLevel = (int)$level;
        return $this;
    }

    public function isDebugLevel($level)
    {
     ...
     * @return $this
     */
    public function setDebugFormat($format)
    {
        ...
        $this->debugFormat = (string)$format;
        return $this;
    }
     ...
     * @return string
     */
    public function getDebugFormat()
    {
     ...
     * @return bool
     */
    public function getLogLoopWait()
    {
     ...
     * @return $this
     */
    public function setLogLoopWait($logLoopWait)
    {
        $this->logLoopWait = (bool)$logLoopWait;
        return $this;
    }
     ...
     * @return $this
     */
    public function setShutdownOnInterrupt($flag)
    {
        $this->shutdownOnInterrupt = (bool)$flag;
        return $this;
    }
     ...
     * @return bool
     */
    public function isShutdownOnInterrupt()
    {
     ...
     * @return bool
     */
    public function isAdvancedLogChecks()
    {
     ...
     * @return $this
     */
    public function setAdvancedLogChecks($flag)
    {
        $this->advancedLogChecks = (bool)$flag;
        return $this;
    }
     ...
     * in production apps this should'nt be too much of a problem.
     *
     * @return bool
     */
    protected function isLogFileChanged()
    {
        if (!$this->logFile) {
            return false;
        }
     ...
     * @return $this
     * @throws CleanErrorException|\RuntimeException
     */
    public function addPlugin($class, $alias = null, $options = [], $lazy = false)
    {
        ...
        try {
            if (!is_object($class) and !$lazy) {
                ...
                $plugin = new $class();
                if (!$plugin instanceof PluginInterface) {
                    throw self::createInvalidArgumentException(func_get_args(), sprintf('Invalid plugin class "%s"; Must implement Lifo\Daemon\Plugin\PluginInterface', $class));
                }
                $plugin->setup((array)$options);
     ...
     * @return string|null
     */
    public function getPluginAlias($class)
    {
        if (is_object($class)) {
     ...
     * @param string          $alias Short alias name for worker. Defaults to snake_case of $worker.
     * @param IPCInterface    $ipc   IPC for parent/worker
     * @return Mediator A new Mediator instance for the worker queue.
     */
    public function addWorker($worker, $alias = null, $ipc = null)
    {
...
//            pcntl_signal_dispatch();
     ...
     * @return string
     */
    public static function findPhp($includeArgs = true)
    {
    ...
    protected function validateEnvironment()
    {
        ...
        if (isset($this->plugins[self::PROCESS_MANAGER_PLUGIN_NAME])) {
            if (!$this->plugin(self::PROCESS_MANAGER_PLUGIN_NAME) instanceof ProcessManagerInterface) {
                $errors[] = sprintf('ProcessManager plugin "%s" must implement Lifo\Daemon\Plugin\ProcessManagerInterface', self::PROCESS_MANAGER_PLUGIN_NAME);
            }
        }
    ...
    public function setPid($pid)
    {
        ...
        if ($this->parent && $initial && $pid != $this->pid) {
            $this->dispatch(DaemonEvent::ON_PID_CHANGE, new PidEvent($this, $pid, $old));
        }
        return $this;
    }
    ...
    /**
     * Is the current process the original parent process?
     * Will be false in any forked processes.
     *
     * @return bool
     */
     ...
     * @return $this
     */
    public function setParent($flag)
    {
        $this->parent = (bool)$flag;
        return $this;
    }
